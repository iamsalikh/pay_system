...
from database.userservice import register_user_db, block_user_db, unblock_user_db, change_user_info_db, check_user_db, add_photo_db
...
async def upload_photo(user_id: int = Body(...), photo_file: UploadFile = None):
    photo_path = f'media/{photo_file.filename}'
    # сохранение пути к файлу
    result = add_photo_db(user_id, photo_path)
...
async def change_info(user_id: int = Body(...), info_to_change: str = Body(...), new_info: str = Body(...)):
    result = change_user_info_db(user_id, info_to_change, new_info)

    return {'status': 1, 'data': result}


# Заблокированные пользователя
